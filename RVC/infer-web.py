...
def cli_infer(com):
    ...
    print("RVC-Fork Infer-CLI: Starting the inference...")
    vc_data = vc.get_vc(model_name, protection_amnt, protect1)
    print(vc_data)
    print("RVC-Fork Infer-CLI: Performing inference...")
    ...
    if "Success." in conversion_data[0]:
        print(
            "RVC-Fork Infer-CLI: Inference succeeded. Writing to %s/%s..."
        ...
        print(
            "RVC-Fork Infer-CLI: Finished! Saved output to %s/%s"
            % ("assets", "audios", "audio-outputs", output_file_name)
        )
    else:
        print("RVC-Fork Infer-CLI: Inference failed. Here's the traceback: ")
...
def cli_extract_feature(com):
    ...
    print("RVC-CLI: Extract Feature Has Pitch: " + str(has_pitch_guidance))
    print("RVC-CLI: Extract Feature Version: " + str(version))
    print("RVC-Fork Feature Extraction: Starting...")
    ...
    execute_generator_function(generator)
    print("RVC-Fork Feature Extraction: Finished")
...
def cli_train(com):
    ...
    print("RVC-Fork Train-CLI: Training...")
    click_train(
        model_name,
        sample_rate,
        has_pitch_guidance,
...
def cli_train_feature(com):
    ...
    model_name = com[0]
    version = com[1]
    print("RVC-Fork Train Feature Index-CLI: Training... Please wait")
    generator = train_index(model_name, version)
    execute_generator_function(generator)
    print("RVC-Fork Train Feature Index-CLI: Done!")
...
def cli_extract_model(com):
    ...
    if extract_small_model_process == "Success.":
        print("RVC-Fork Extract Small Model: Success!")
    else:
        print(str(extract_small_model_process))
        print("RVC-Fork Extract Small Model: Failed!")
...
def GradioSetup():
    ...
    with gr.Blocks(theme=my_applio, title="RVC-Fork") as app:
        gr.HTML("<h1> 🍏 RVC-Fork </h1>")
        with gr.Tabs():
