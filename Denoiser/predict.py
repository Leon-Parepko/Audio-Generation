...
FIRST_MODEL_OUTPUT_ROOT = os.path.join(PROJECT_ROOT, "../Interval_Detector/model_output", FIRST_MODEL_EXP_NAME, "outputs")
...
def evaluate(args, data_list_info, save_individual_results=True, save_stat=True):
    ...
    if torch.cuda.device_count() > 1:
        ...
        net = torch.nn.DataParallel(net.cuda())   # For multi-GPU
    else:
...
def main():
    ...
    if not args.eval:
        ...
        if not args.unknown_clean_signal:
            ...
            data_list_info = get_data_from_first_model(first_model_json_path, snr=args.snr)
            ...
            # print(os.path.exists(first_model_json_path))
            # print(args.outputs)
            data_list_info = get_data_from_first_model(first_model_json_path, snr=args.snr, unknown_clean_signal=args.unknown_clean_signal)
            evaluate(args, data_list_info, save_individual_results=args.save_results, save_stat=True)
